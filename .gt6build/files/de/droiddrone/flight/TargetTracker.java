...
/**
 * Lightweight grayscale template tracker designed for phone-side target-lock tests.
 * It has no Android dependencies and never sends flight-control commands.
 */
public final class TargetTracker {
    ...
    private double templateSum, templateSqSum;
    ...
    public synchronized void requestLock(float centerX, float centerY, float width, float height) {
        ...
        requestedW = clamp(width, 0.04f, 0.7f);
        requestedH = clamp(height, 0.04f, 0.7f);
        lockPending = true;
    ...
    public synchronized Result process(byte[] yPlane, int width, int height, int rowStride) {
        frameId++;
        if (yPlane == null || width < 16 || height < 16 || rowStride < width) return result;
        ...
        int baseRadiusX = Math.max(6, Math.round(templateW * 0.65f));
        int baseRadiusY = Math.max(6, Math.round(templateH * 0.65f));
        float expansion = 1f + Math.min(2.0f, lostFrames * 0.20f);
        ...
        double bestCorr = -1.0;
        int bestX = Math.round(cx - templateW * 0.5f);
        int bestY = Math.round(cy - templateH * 0.5f);
        ...
        if (confidence >= 0.48f) {
            float dx = matchCx - cx;
            float dy = matchCy - cy;
            vx = vx * 0.62f + dx * 0.38f;
            vy = vy * 0.62f + dy * 0.38f;
            cx = matchCx;
            cy = matchCy;
            lostFrames = 0;
            if (confidence >= 0.78f) updateTemplate(bestX, bestY, 0.04f);
    ...
    private void downsample(byte[] src, int width, int height, int rowStride) {
        ...
        int dst = 0;
        for (int y = 0; y < h; y++) {
            int sy0 = y * downsample;
            int sy1 = Math.min(height, sy0 + downsample);
            for (int x = 0; x < w; x++) {
                int sx0 = x * downsample;
                int sx1 = Math.min(width, sx0 + downsample);
                int sum = 0;
                int count = 0;
                ...
                reducedFrame[dst++] = (byte) (sum / Math.max(1, count));
            }
        }
    }
    ...
    private double correlationAt(int x0, int y0) {
        ...
        long sumC = 0, sumCSq = 0, sumTC = 0;
        int n = 0;
        long sumT = 0, sumTSq = 0;
