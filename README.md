...
### Pit Lane Screen
RaceLab 2.5.0 дублирует PIT-данные на второй экран команды через локальную сеть.

- Встроенный HTTP-сервер: порт `8787`.
- Второму устройству не нужен APK: достаточно браузера.
- Подключить телефон гонщика и планшет/ноутбук команды к одной Wi‑Fi сети либо включить hotspot на телефоне гонщика.
- В RaceLab открыть `Настройки → PIT LANE SCREEN`.
- Открыть показанный адрес, например `http://192.168.43.1:8787`.
- Экран команды обновляется каждые 250 мс (~4 Гц).
- Показывает:
  - PIT ACTIVE / READY;
  - текущее PIT TIME;
  - LAST;
