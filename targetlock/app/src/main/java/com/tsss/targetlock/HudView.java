...
public class HudView extends View {
    ...
    @Override protected void onDraw(Canvas c){
        ...
        int dim=Color.argb(175,112,255,125),panel=Color.argb(150,0,12,4);

        p.setColor(panel);p.setStyle(Paint.Style.FILL);
        c.drawRoundRect(12,10,820,190,12,12,p);
        ...
        c.drawText(String.format("%s   TRK %.0f FPS / %.1f ms",state,t.trackerFps,t.latencyMs),24,34,p);
        ...
        c.drawText(ys,24,62,p);

        String cal=imu.calibrating
                ?String.format("CAL %3.0f%%",imu.calibrationProgress*100f)
                :"CAL OK";
        c.drawText(String.format("IMU %.0f Hz  %.1f deg/s  %s  Kx %.2f Ky %.2f",
                imu.gyroHz,imu.angularSpeedDeg,cal,imu.strengthX,imu.strengthY),24,90,p);
        ...
        if(t.locked||t.acquiring){
            ...
            c.drawText("AUTO YOLO26n + IMU/FLOW FUSION  •  tap = manual lock",24,h-28,p);
        }
