...
public class HudView extends View {
    ...
    private final RectF[] buttons=new RectF[4];
    private final String[] labels={"DETECT","HOLD","REACQ","CLEAR"};
    ...
    @Override protected void onDraw(Canvas c){
        ...
        c.drawText(String.format("CAM %.0f/%d FPS  %s  ANALYSIS %dx%d",
                ...
                camera.getAnalysisW(),camera.getAnalysisH()),24,58,p);
        ...
        if(t.locked||t.acquiring||t.coasting||t.reacquiring){
            float tx=t.renderX*w,ty=t.renderY*h;
            float s=Math.max(18f,Math.min(w,h)*t.box);
            p.setStyle(Paint.Style.STROKE);
            p.setStrokeWidth(3.4f);
            p.setColor(t.locked?green:amber);
            drawCorners(c,tx-s,ty-s,tx+s,ty+s,16,p);
            c.drawCircle(tx,ty,6,p);
        }
